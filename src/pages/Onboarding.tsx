...
export function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  const pages = [<><span className="onboarding-icon">◉</span><h1>记录每一次视觉异常</h1><p>实时计时或事后补录，帮助你整理变化与就诊材料。</p></>, <><span className="onboarding-icon">⌂</span><h1>本地优先，可选同步</h1><p>无需账号即可使用；注册或登录后，可以把本地记录同步到你的账户。</p><label className="accept"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} />我知道本地模式需要自行备份</label></>, <><span className="onboarding-icon">＋</span><h1>添加到主屏幕</h1><p>在 Safari 点“分享”，选择“添加到主屏幕”。首次加载后可离线使用。</p></>];
