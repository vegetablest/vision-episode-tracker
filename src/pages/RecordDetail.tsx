...
export function RecordDetail() {
  ...
  return <main className="page narrow"><header><p className="eyebrow">记录详情</p><h1>{episode.status === "voided" ? "已作废的记录" : episode.status === "ongoing" ? "正在发生" : "视觉异常记录"}</h1></header><section className="detail-card">{rows.map(([key, value]) => <div key={key}><span>{key}</span><strong>{value}</strong></div>)}</section><div className="button-row"><button className="secondary" onClick={toggleVoid}>{episode.status === "voided" ? "恢复记录" : "作废记录"}</button><button className="text-danger" onClick={remove}>永久删除测试数据</button></div><p className="medical-note">本应用只整理你记录的事实，不提供诊断或治疗建议。</p></main>;
}
