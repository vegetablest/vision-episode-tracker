...
export function Trends() {
  ...
  const eyeScopes = [...episodes.filter((item) => item.status !== "voided" && item.eyeScope).reduce((map, item) => map.set(item.eyeScope!, (map.get(item.eyeScope!) ?? 0) + 1), new Map<string, number>())].sort((a, b) => b[1] - a[1]);
  const associated = [...episodes.filter((item) => item.status !== "voided").flatMap((item) => item.associatedSymptoms).reduce((map, item) => map.set(item, (map.get(item) ?? 0) + 1), new Map<string, number>())].sort((a, b) => b[1] - a[1]);
  return <main className="page"><header><p className="eyebrow">趋势</p><h1>看见变化，不下结论</h1><p>样本量较小时，仅展示分布，不推断高风险时段。</p></header><div className="stat-banner"><div><strong>{metrics.episodeCount}</strong><span>近 30 天次数</span></div><div><strong>{metrics.symptomDays}</strong><span>症状日</span></div><div><strong>{metrics.medianDurationMinutes === null ? "—" : Math.round(metrics.medianDurationMinutes)}</strong><span>中位分钟</span></div></div>
    ...
    <section><h2>眼别分布</h2><div className="tag-counts">{eyeScopes.map(([name, count]) => <span key={name}>{labels[name]} <strong>{count}</strong></span>)}{!eyeScopes.length && <p className="hint">记录眼别后显示。</p>}</div></section>
    <section><h2>伴随表现</h2><div className="tag-counts">{associated.map(([name, count]) => <span key={name}>{labels[name]} <strong>{count}</strong></span>)}{!associated.length && <p className="hint">展开“记录更多信息”填写后显示。</p>}</div></section>
