...
import type { AppMetadata, AppSettings, VisionEpisode } from "../domain/episode";
...
export async function initializeDatabase(database = db): Promise<void> {
  ...
  await database.transaction("rw", database.settings, database.metadata, async () => {
    ...
    if (!await database.metadata.get("metadata")) await database.metadata.add({ id: "metadata", databaseSchemaVersion: 1, createdAt: now, lastMigrationAt: null, lastBackupAt: null, changesSinceBackup: 0 });
  });
}
